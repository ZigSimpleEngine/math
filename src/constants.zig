...
/// Compile-time cast of a decimal literal into `T`. Using `@as` (not a
...
/// Machine epsilon of `T` (GLM `epsilon<T>()`): the smallest ε such that
/// `1 + ε != 1`. Use it as the default tolerance in equality helpers:
/// `equalEps(a, b, constants.epsilon(T))`.
...
/// cos(1/2) ≈ 0.8776 (GLM `cos_one_over_two<T>()`): the cosine of half a
...
/// Additive identity of `T`: `x + zero(T) == x`. Exists for code that
/// builds constants generically over scalar types.
pub fn zero(comptime T: type) T {
    return cast(T, 0);
}

/// Multiplicative identity of `T`: `x * one(T) == x`.
...
/// τ — alias of `two_pi` (GLM `tau<T>()`): the modern name for the
/// circle constant; provided for GPU-shader parity.
pub fn tau(comptime T: type) T {
    return two_pi(T);
}

/// √π (GLM `root_pi<T>()`): the normalization constant of the Gaussian
...
/// Euler–Mascheroni constant γ ≈ 0.5772 (GLM `euler<T>()`, also known as
...
/// √2 ≈ 1.4142 (GLM `root_two<T>()`): the diagonal length of a unit
/// square.
pub fn root_two(comptime T: type) T {
    return cast(T, 1.41421356237309504880168872420969808);
}

/// √3 ≈ 1.7321 (GLM `root_three<T>()`): diagonal of a unit cube and the
